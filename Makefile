...
LDFLAGS ?= -X github.com/chrislusf/seaweedfs/weed/util.COMMIT=${COMMIT} -X github.com/chrislusf/seaweedfs/weed/util.PRIVATE_VERSION=${PRIVATE_VERSION}
...
linux: deps
	mkdir -p linux
	GOOS=linux GOARCH=amd64 CGO_ENABLED=1 go build $(GO_FLAGS) -ldflags "$(LDFLAGS)" -o linux/$(BINARY) $(SOURCE_DIR)
	GOOS=linux GOARCH=amd64 CGO_ENABLED=1 go build $(GO_FLAGS) -tags 5BytesOffset -ldflags "$(LDFLAGS)" -o linux/$(BINARY)-large-disk $(SOURCE_DIR)
